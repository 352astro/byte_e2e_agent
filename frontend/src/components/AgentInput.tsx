...
export default function AgentInput({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    setRows(Math.min(Math.max(e.target.value.split("\n").length, 1), MAX_ROWS));
  };
  ...
  const handleSendClick = () => {
    if ((running || runtimeBusy) && !interrupting) {
      onInterrupt();
      return;
    }
    if (prefillContent.trim()) {
            ...
            <div className="agent-send-queue-header">
              ...
              <div className="agent-send-queue-actions">
                <button
                  ...
                  disabled={
                    interrupting || (!queuedSends.length && !question.trim())
                  }
                >
                  Send now
                </button>
              ...
              {renderedQueue.map((item) => (
                <div
                  ...
                  className={`agent-send-queue-row${
                    item.exiting ? " agent-send-queue-row--exiting" : ""
                  }`}
                >
                  ...
                  <textarea
                    ...
                    onChange={(e) => {
                      onUpdateQueue(item.index, e.target.value);
                    }}
                  />
                </div>
              ))}
